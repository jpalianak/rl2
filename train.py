...
def train_dqn_for_ticker(ticker, df_train, max_episodes=config.MAX_EPISODES):
    env = TradingEnv(df=df_train, window_size=10)
    n_observations = env.observation_space.shape[0]  # type: ignore
    ...
    for episode in range(max_episodes):
        ...
        for step in range(config.MAX_STEPS_PER_EPISODE):
            ...
            replay_buffer.append(Transition(
                state, action, reward_tensor, next_state, done_tensor))
            state = next_state
            ...
            if len(replay_buffer) >= config.BATCH_SIZE:
                ...
                with torch.no_grad():
                    ...
                    selected_target_next_q_values = target_next_q_values.gather(
                        1, online_best_next_actions).squeeze(1)
        ...
        epsilon = max(config.EPSILON_END,
                      config.EPSILON_START - (episode / config.EPSILON_DECAY_EPISODES) *
                      (config.EPSILON_START - config.EPSILON_END))

        if (episode + 1) % config.PRINT_EVERY == 0:
            avg_reward = np.mean(episode_rewards_history[-config.PRINT_EVERY:])
            print(f'[{ticker}] Episodio: {episode + 1}/{max_episodes}, Recompensa promedio: {avg_reward:.2f}, Epsilon: {epsilon:.3f}')
...
def load_model(ticker, input_dim, output_dim):
    model = create_q_network(input_dim, output_dim)
